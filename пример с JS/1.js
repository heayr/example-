let phrases = [
  ...
  for (let i=0; i <= 2; i = i + 1){ // здесь необходимо оставить 'i = i + 1 чтобы не было бесконечного перебора'
    smoothly(phrase, 'textContent', phrases[i].text); // всегда нужно обращаться к массиву в данном случае 'phrases'
    smoothly(image, 'src', phrases[i].image); // всегда нужно обращаться к массиву в данном случае 'phrases' затем к 
    // предмету .text или .image ; [i] для регулировки количества обращений
  }